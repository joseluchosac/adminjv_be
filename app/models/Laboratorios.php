...
class Laboratorios
{
  static public function filterLaboratorios($campos, $where, $orderBy, $pagination, $isPaginated = true){
    $table = "laboratorios";
  ...
  static function getLaboratorios($tabla, $campos, $whereEquals = null)
  {
  ...
  static function countRecordsBy($equal, $exclude = []){
    ...
    $sql = "SELECT COUNT(*) AS count FROM laboratorios" . $where;
  ...
  static function createLaboratorio($params)
  {
    $sql = sqlInsert("laboratorios", $params);
    $dbh = Conexion::conectar();
    $stmt = $dbh->prepare($sql);
    $stmt->execute($params);
  ...
  static function updateLaboratorio($table, $paramCampos, $paramWhere)
  {
  ...
  static function deleteLaboratorio($params){
  ...
  static function getLaboratorio($id){
    $sql = "SELECT id, nombre, estado FROM laboratorios WHERE id = :id;";
    $dbh = Conexion::conectar();
    $stmt = $dbh->prepare($sql);
    $stmt->execute(['id' => $id]);
